...
export async function getAvailableGenomes() {
  const apiUrl = "/genome-api/list/ucscGenomes";
...
export async function getGenomeChromosomes(genomeId: string) {
  const apiUrl = `/genome-api/list/chromosomes?genome=${genomeId}`;
  const response = await fetch(apiUrl);
  if (!response.ok) {
...
export async function searchGenes(query: string, genome: string) {
  const url = "/clinical-tables-api/api/ncbi_genes/v3/search";
...
export async function fetchGeneDetails(geneId: string): Promise<{
  ...
  try {
    const detailUrl = `/ncbi-api/entrez/eutils/esummary.fcgi?db=gene&id=${geneId}&retmode=json`;
...
export async function fetchGeneSequence(
  ...
  try {
    ...
    const apiUrl = `/genome-api/getData/sequence?genome=${genomeId}&chrom=${chromosome}&start=${apiStart}&end=${apiEnd}`;
    const response = await fetch(apiUrl);
    const data = await response.json();
...
export async function fetchClinvarVariants(
  ...
  const searchUrl = "/ncbi-api/entrez/eutils/esearch.fcgi";
  ...
  const summaryUrl = "/ncbi-api/entrez/eutils/esummary.fcgi";
...
export async function analyzeVariantWithAPI({
  ...
  const url = `${process.env.NEXT_PUBLIC_ANALYZE_SINGLE_VARIANT_BASE_URL}?${queryParams.toString()}`;
